...
module.exports = {
  ...
  create: context => {
    ...
    const containsThis = node => {
      return sourceCode
        .getTokens(node)
        .some(token => token.type === 'Keyword' && token.value === 'this');
    };
    ...
    const getMessage = node => {
      if (singleReturnOnly && returnsImmediately(node)) {
        return 'Prefer using arrow functions over plain functions which only return a value';
      }
      if (getFunctionName(node)) {
        return 'Use const or class constructors instead of named functions';
      }
      return 'Prefer using arrow functions over plain functions';
    };
